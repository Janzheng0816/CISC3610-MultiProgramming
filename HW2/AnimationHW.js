var spritesheet = {
	"frames": [
		{
			"frame": {
				"x": 1,
				"y": 1,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 253,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 505,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 757,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 1009,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 1261,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 1513,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 1765,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 2017,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 2269,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 2521,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 2773,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 3025,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"frame": {
				"x": 1,
				"y": 3277,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"filename": "tile016.png",
			"frame": {
				"x": 1,
				"y": 3529,
				"w": 498,
				"h": 250
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 250
			},
			"sourceSize": {
				"w": 498,
				"h": 250
			}
		},
		{
			"filename": "tile018.png",
			"frame": {
				"x": 1,
				"y": 3781,
				"w": 498,
				"h": 248
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 248
			},
			"sourceSize": {
				"w": 498,
				"h": 248
			}
		},
		{
			"filename": "tile019.png",
			"frame": {
				"x": 1,
				"y": 4031,
				"w": 498,
				"h": 248
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 248
			},
			"sourceSize": {
				"w": 498,
				"h": 248
			}
		},
		{
			"filename": "tile020.png",
			"frame": {
				"x": 1,
				"y": 4281,
				"w": 498,
				"h": 248
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 248
			},
			"sourceSize": {
				"w": 498,
				"h": 248
			}
		},
		{
			"filename": "tile021.png",
			"frame": {
				"x": 1,
				"y": 4531,
				"w": 498,
				"h": 248
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 248
			},
			"sourceSize": {
				"w": 498,
				"h": 248
			}
		},
		{
			"filename": "tile022.png",
			"frame": {
				"x": 1,
				"y": 4781,
				"w": 498,
				"h": 248
			},
			"rotated": false,
			"trimmed": false,
			"spriteSourceSize": {
				"x": 0,
				"y": 0,
				"w": 498,
				"h": 248
			},
			"sourceSize": {
				"w": 498,
				"h": 248
			}
		}
	]
}

var myNum = new Image();
myNum.src = './spritesheet.png';


const myCanvas = document.getElementById('canvas');
const ctx = myCanvas.getContext('2d');


let button = document.querySelector("button");
var flag =0;
button.onclick = function(){
	 draw();
	 flag=0;
	 
 }
var background = new Image();
	background.src = './office.png';
	background.onload = function(){
		ctx.drawImage(background, 0, 0);
	}

function drawNum(i){
	ctx.drawImage(myNum, spritesheet.frames[i].frame.x,spritesheet.frames[i].frame.y, spritesheet.frames[i].frame.w, spritesheet.frames[i].frame.h, 20, 50, spritesheet.frames[i].sourceSize.w-100, spritesheet.frames[i].sourceSize.h-100);
}


//function drawNum2(x,y){
	//ctx.drawImage(myNum,spritesheet.frames[x].frame.x,spritesheet.frames[x].frame.y, spritesheet.frames[x].frame.w, spritesheet.frames[x].frame.h, 150, 190, spritesheet.frames[x].sourceSize.w, spritesheet.frames[x].sourceSize.h);
	//ctx.drawImage(myNum,spritesheet.frames[y].frame.x,spritesheet.frames[y].frame.y, spritesheet.frames[y].frame.w, spritesheet.frames[y].frame.h, 250, 200, spritesheet.frames[y].sourceSize.w, spritesheet.frames[y].sourceSize.h);
//}
function draw(){
   if (flag <= 19){
     //ctx.clearRect(30,50, myCanvas.width, myCanvas.height);
     drawNum(flag);
     setTimeout(draw, 100);
	 flag++;
	 if (flag == 19)
		 flag = 0;
   } //else
		
	   //if (flag = 9){
		// ctx.clearRect(0,0, myCanvas.width, myCanvas.height);
		// drawNum2(0,9);
    // setTimeout(draw, 500);
	// }
  
 // flag++
}